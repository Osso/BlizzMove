...
test("IsProtected returns isProtectedFromInsecure=true only in combat", function()
    local f = make_test_frame("LockdownInsecureTest", true)
    -- Out of combat: first return is false
    local fromInsecure, isProtected = f:IsProtected()
    assertFalse(fromInsecure)
    assertTrue(isProtected)
    -- In combat: first return is true (we are addon code = insecure)
    A_Admin.SetInCombat(true)
    fromInsecure, isProtected = f:IsProtected()
    assertTrue(fromInsecure)
    assertTrue(isProtected)
    A_Admin.SetInCombat(false)
end)
